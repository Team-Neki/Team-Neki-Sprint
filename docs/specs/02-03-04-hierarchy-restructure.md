...
- **규모**: XL · **스키마 변경**: 대(모델 신설·전환 + 데이터 리셋 재시드) · **상태**: DONE(`feat/hierarchy-restructure` 구현 완료)
...
## 열린 항목 → 확정 (2026-07-08 핑 8)

- **기존 데이터**: 백필/마이그레이션 아님 → **기존 더미 데이터 정리(리셋)** 후 새 구조로 재시드. `prisma migrate reset` + 새 `seed.ts`. (구 Epic/Task 팀 백필·재번호 불필요.)
- **Project key**: 없음. **제목만**으로 충분(번호/접두어 미부여).
- **유저 팀 배정**: 관리 UI 수동 배정만(시드 미반영). 시드에는 팀만 생성.

> 이로써 마이그레이션이 크게 단순화됨: Initiative→Project 데이터 보존 대신 도메인 데이터 리셋 + 재시드.
