...
- **본문 폭(BACKEND-145)**: 읽기 뷰·헤더·하단 댓글은 `max-w-5xl`, 에디터만 `3xl`. 댓글 거터(296px)가 5xl 컨테이너 기준이라 에디터를 5xl 로 맞췄다. 툴바 `sticky top-14`(56px)는 헤더 실측 높이(py-2 + h-7/h-8 버튼 + border ≈ 45~49px)보다 커 그 틈으로 본문이 비쳤다 → 후속 PR 에서 헤더가 `ResizeObserver` 로 자기 높이를 `--wiki-header-h` CSS 변수로 게시하고 툴바가 `top-[var(--wiki-header-h,3.5rem)]` 로 읽는다(고정 px 대신 실측 추종, 공지 에디터는 폴백 3.5rem).
