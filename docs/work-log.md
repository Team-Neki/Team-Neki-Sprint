...
| 날짜 | 세션 | 상태 |
|---|---|---|
| 2026-08-01 | 엔티티 4화면 UX 정합성(BACKEND-47/48/49): 같은 계층·같은 역할인데 화면마다 조작이 갈리던 부채 정리. (1) **스프린트 상세 규격 이관** — 2단 grid·인라인 편집(`name`/`endDate`/`SprintStatus` 필드 차이는 prop 으로 흡수)·`updateSprintFields`(diff+`field_changed` 로깅)·업무 히스토리 노출(로그는 쌓이는데 화면이 없었음)·`@detail` 슬라이드 상세·설명 카드 상시 렌더, (2) **상세 정합성** — 댓글/히스토리를 공용 `CommentsHistoryTabs` 탭으로 통일(태스크 탭 vs 에픽·프로젝트 카드나열 vs 스프린트 누락), 프로젝트 상세 라벨 행 추가(`getProject` 가 자기 labels 미포함이라 목록에서 단 라벨이 상세에서 사라져 보였음), 태스크 상세 grid `min-w-0` 누락 보정, 뒤로가기·삭제 후 이동 `/board`→`/tasks`, (3) **목록/표 정합성** — 정렬 헤더를 에픽·태스크·스프린트로 확대(`parseListSort`+`listSortOrderBy` 공용화, +vitest 4), 스프린트만 쓰던 `EmptyState` 분기 제거(표 유지 규칙으로), MD 컬럼 표기 통일 + 프로젝트 표 MD 롤업 추가, 스프린트 표 열기 아이콘. 규격은 [design-system.md](./design-system.md#엔티티-목록상세-화면-규격)에 명문화 | `DONE`\* |
...
- **npm 게시 파이프라인**: `.github/workflows/publish-mcp.yml` — `mcp/**` 가 main 에 반영되면 test+build 후 npm 에 없는 버전일 때만 `npm publish`(버전 bump 머지 = 게시 의사표시, 동일 버전은 no-op). 수동 재시도는 workflow_dispatch. 루트 CI 가 `mcp/` 를 안 돌리므로 이 워크플로우가 mcp 검증도 겸한다. **레포 시크릿 `NPM_TOKEN`(automation token) 등록 필요.**
...
- **같은 부류 전수 조사(⌘K 팔레트 1건 추가 수정)**: 오버레이·가로overflow 전반을 훑은 결과, 넓은 콘텐츠(표 `ui/table.tsx`·칸반 `kanban.tsx:217`·타임라인 `epic-timeline.tsx:253`)는 모두 이미 `overflow-x-auto` 래퍼가 있었고, 라이트박스(`max-h-full max-w-full object-contain`)·버전기록(`h-[70vh]` — 항상 뷰포트 내)도 안전. **유일한 추가 버그는 커맨드 팔레트**(`command-palette.tsx`): `top-[15vh]` 로 상단 오프셋을 주면서 높이 상한은 `DialogContent` 기본값 `max-h-[calc(100dvh-2rem)]` 을 그대로 써서, 오프셋만큼 하단이 화면 밖으로 나갔다(짧은 뷰포트에서만 발현 — 높이 386px 에서 25px, 300px 에서 12px 잘림 실측). `top-[15dvh] max-h-[calc(85dvh-1rem)] overflow-x-hidden` 으로 수정 → 세 뷰포트 모두 잘림 0, 844px 에선 렌더 결과 동일(회귀 없음). **교훈: 중앙 정렬이 아닌 오프셋 배치 팝업은 높이 상한도 오프셋을 뺀 값이어야 한다.**
