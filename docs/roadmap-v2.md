...
| F1 | iOS Safari 인라인 댓글 **작성**(터치 선택) | F | S~M | `REVIEW` (실기기 재현 확인 선행) |

**열린 항목은 D1(테스트 통합/E2E 인프라) · D4(SSE, 의도적 보류) · F1(2026-08-01 추가).** 나머지 전부 `DONE`.
...
### F1(신규). iOS Safari 위키 인라인 댓글 **작성**(터치 선택) · 규모 S~M · `REVIEW`

> **먼저 재현 확인이 필요하다.** 아래는 코드에서 추론한 위험이지 관측된 버그가 아니다. 아이폰에서 위키 본문을 롱프레스로 선택했을 때 플로팅 '댓글' 버튼이 뜨면 이 항목은 닫는다.

- **현상/배경**: 인라인 댓글 **읽기**는 2026-08-01(BACKEND-54)에 앵커 아래 팝오버로 해결됐으나, **작성** 경로는 손대지 않았다. 선택 감지가 `document.addEventListener("mouseup")` **하나뿐**(`wiki-comments-view.tsx:177`)인데, iOS Safari 는 롱프레스 텍스트 선택·선택 핸들 드래그에서 `mouseup` 을 일관되게 발화하지 않는다 → 선택해도 플로팅 '댓글' 버튼이 안 뜰 수 있다.
- **접근안**: `mouseup` → **`selectionchange`**(iOS Safari 9+) 전환. 단순 교체가 아니라 세 가지가 딸려온다.
  - (a) **디바운스 필수** — 선택 핸들 드래그 중 연속 발화. 150ms 정도 or rAF 로 묶는다.
  - (b) **탭이 선택을 지우는 경합** — '댓글' 버튼을 탭하는 순간 selection 이 collapse 되고 `selectionchange` 가 먼저 발화해 클릭 도달 전에 버튼이 사라질 수 있다. 현재 line 446 의 `onMouseDown preventDefault` 는 데스크톱 전용 트릭이라(터치는 mousedown 이 뒤늦게 합성됨) 못 막는다 → **우리 UI 안에서 pointer 가 눌린 동안은 composer 를 비우지 않는 가드**가 필요. 다행히 `from`/`to` 는 선택 시점에 composer state 로 스냅샷되므로 커밋 시 live selection 은 불필요하다 — 위험이 '버튼이 사라지는 것' 하나로 줄어든다.
  - (c) **iOS 네이티브 콜아웃과 자리싸움** — 복사/찾아보기/공유 말풍선이 선택 영역 **바로 위**에 뜨는데 '댓글' 버튼도 `-translate-y-full` 로 정확히 거기 있다. 터치에선 선택 **아래**로 뒤집어야 한다.
- **영향 파일**: `src/components/wiki/wiki-comments-view.tsx`(선택 감지 `onMouseUp`/리스너·composer 위치·`onMouseDown` 가드).
- **데이터 모델 영향**: 없음.
- **열린 질문**:
  - 데스크톱도 `selectionchange` 로 통일할지, `mouseup` 을 남기고 터치만 추가할지. (통일이 깔끔하지만 잘 도는 데스크톱 경로를 건드리는 리스크)
  - 콜아웃 회피를 '아래로 뒤집기'로 할지, 롱프레스 대신 **앵커 없는 페이지 댓글**로 유도할지(모바일에선 인라인 작성 자체를 접는 선택지).
- **검증 제약(중요)**: 이건 **실기기 문제**다. iframe 복제 측정(CLAUDE.md 반응형 검증법)은 CSS 기하만 보지 선택 동작의 의미론은 못 본다. Chrome 기기 에뮬레이션도 **Blink 선택 구현을 그대로 쓰므로 WebKit 롱프레스 동작을 재현하지 않는다.** 실제 아이폰 확인이 필수 — 구현보다 검증이 병목.
