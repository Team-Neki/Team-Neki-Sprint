...
- **1단계 병렬(worktree) 착수**: #1 타임라인, #5+#6 상태바, #7 필터, #8 위키 → 스펙([`specs/`](./specs/)) 작성 후 `feat/*` 브랜치에서 병렬 구현 중(`WIP`).
- **#2**: [`adr/0001-initiative-parent.md`](./adr/0001-initiative-parent.md)로 검토·권장안 작성, 사용자 확정 대기(`REVIEW`).
- **2단계 보류**: #3(커스텀 key/Project), #4(유저 그룹) — 스키마·마이그레이션 동반 + 열린 질문 확정 필요라 1단계 병합·안정화 후 순차 진행.
