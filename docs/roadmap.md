...
- **B6 리치 입력 확장** (`DONE` 2026-07-08): 설명(description)·댓글에서 링크(`#`티켓)/멘션(`@`사람) 동작. plain textarea → Tiptap(`wikiExtensions` 재사용). **저장은 기존 String 컬럼에 doc JSON 문자열**(스키마 변경 없음, 레거시 plain text 는 `parseDoc` 폴백). 신규 `rich-text/rich-editor.tsx`(`RichEditor` 편집 + `RichContent` 읽기전용), `lib/rich-content.ts`(parseDoc·plainTextOf·mentionsInValue), `server/notify.ts`(`notifyNewMentions` 공용). 설명(`InlineDescription`)·댓글(`comment-form`·표시) 교체. 댓글/설명의 새 `@`멘션 → 알림(`addComment`·`update*Fields`). 히스토리는 설명 JSON 을 plain text 로 발췌(`activity-format`). 생성 다이얼로그의 설명은 plain textarea 유지(폴백으로 호환).
